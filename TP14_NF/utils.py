...
def check(flow, x, eps):
    assert (x - flow.f(flow.invf(x)[0])[0]).mean() < eps
...
def scatterplots(samples: List[Tuple[str, torch.Tensor]], col_wrap=4):
    ...
    g = sns.FacetGrid(data, height=2, col_wrap=col_wrap,
                      col="origin", sharex=False, sharey=False)
...
class FlowModules(FlowModule):
    ...
    def invf(self, y):
        zs, logdet = self.apply(reversed(self.flows),
                                lambda m, y: m.invf(y), y)
        return zs, logdet
...
class FlowModel(FlowModules):
    ...
    def f(self, x):
        ...
        zs, logdet = super().f(x)  # changed here forward to f
        logprob = self.prior.log_prob(zs[-1])
        return logprob, zs, logdet

    def invf(self, x):
        # Just computes the prior
        zs, logdet = super().invf(x)  # changed here forward to f
        logprob = self.prior.log_prob(zs[-1])
        return logprob, zs, logdet
