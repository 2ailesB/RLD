"""
    This file contains functions and script to execute the PPO clipped algorithm. 
...
class A2C(object):
    ...
    def __init__(self, env, opt):
        ...
        # Actor Critic
        self.Actorlr = 0.01  # actor learning rate
        self.Valuelr = 0.01  # critic learning rate
        self.policy = NN(self.featureExtractor.outSize, self.action_space.n, layers=[
    ...
    def learn(self):
        ...
        gradJ_Theta = 0
        piTheta_k = self.policy(starts).view(-1, self.nbAction).detach() # not changed during loop

        for i in range(self.K):

            self.ActorOptimizer.zero_grad()
            piTheta = self.policy(starts).view(-1, self.nbAction)
            # gradJ_Theta = ((piTheta.gather(1, actions.unsqueeze(1)) - (piTheta_k.gather(1, actions.unsqueeze(1)))) * achap).mean()
            # gradJ_Theta = - gradJ_Theta # gradient ascent
            # # Update policy
            # gradJ_Theta.backward()
            ratios = (piTheta.gather(1, actions.unsqueeze(1))) / (piTheta_k.gather(1, actions.unsqueeze(1)))
            gradJ_Theta = ratios * achap
            clipped_gradJ_Theta = torch.clamp(ratios, 1 - self.epsilon, 1 + self.epsilon) * achap
            Lclip = - torch.min(gradJ_Theta, clipped_gradJ_Theta).mean()
            Lclip.backward()
...
if __name__ == '__main__':
    env, config, outdir, logger = init(
        './configs/config_PPOclipped_lunar.yaml', "PPO_clipped")
