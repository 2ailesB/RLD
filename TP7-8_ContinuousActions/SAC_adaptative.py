...
class SACAdaptative(object):
    """
        class that defines the agent using SAC with adaptative temperature to play
    """
    ...
    def __init__(self, env, opt):
        ...
        self.lrPolicy = opt.ActorLr  # le for policy
        ...
        self.alphaInit = opt.alpha_init
        self.alpha = nn.Parameter(torch.tensor(self.alphaInit, dtype=torch.float32),
                                      requires_grad=True)
        self.target_entropy = opt.target_entropy
        self.lrAlpha = opt.lrAlpha
        self.alphaOptimizer = torch.optim.Adam([self.alpha], lr=self.lrAlpha)
    ...
    def learn(self):
        ...
        # Update alpha
        astarts, probstarts = self.getAction(starts)
        alphal = - (self.alpha * (torch.log(probstarts) + self.target_entropy).detach()).mean() # .detach() autour de la somme ? 
        ...
        return self, qloss1, qloss2, policyl, alphal
...
if __name__ == '__main__':
    env, config, outdir, logger = init(
        './TP7-8_ContinuousActions/configs/config_pendulum-v0_SAC.yaml', "SACAdaptative")
    ...
    agent = SACAdaptative(env, config)
    ...
    for i in range(episode_count):
        ...
        while True:
            ...
            # print('act', action)
            # action.view(agent.action_space_n).detach().numpy()
            new_ob, reward, done, _ = env.step(
                action.view(agent.action_space_n).detach().numpy())  # execute agent
            ...
            if agent.timeToLearn(done):
                _, q1loss, q2loss, ploss, aloss = agent.learn()

            if done:
                if i % agent.freqVerbose == 0:
                    print(str(i) + " rsum=" + str(rsum) + ", " + str(j) + " actions, Q1net loss=" +
                          str(q1loss) + " Q2net loss=" + str(q2loss) + " policy loss=" + str(ploss) + " alpha loss=" + str(aloss))
                ...
                logger.direct_write("alpha loss", aloss, i)
                agent.nbEvents = 0
                mean += rsum
                rsum = 0
